...
module Shanty
  RSpec.describe(Config) do
    ...
    describe('#respond_to?') do
      it('is able to repond to a method on the config class') do
        expect(subject.respond_to?(:merge!)).to be(true)
      end

      it('is able to respond to a method on the underlying openstruct') do
        expect(subject.respond_to?(:nic)).to be(true)
      end

      it('is not able to respond to a non-existent method') do
        expect(subject.respond_to?(:copolla)).to be(false)
      end
    end
  end
end
